    ...
    <div className="wizard-container" style={{margin:0, padding:0}}>
          ...
          <tbody>
            {formularios.map(f => (
              <tr key={f.id_formulario}>
                ...
                <td>
                  <button onClick={() => handleEdit(f)} className="btn" style={{marginRight:8, background:'#fff', color:'#1976d2', border:'1.5px solid #1976d2'}}>Editar</button>
                  <button onClick={() => handleDelete(f.id_formulario)} className="btn-eliminar">Eliminar</button>
                </td>
              </tr>
            ))}
      ...
      {editId && (
        <form onSubmit={handleSubmit} className="wizard-step" style={{maxWidth:400, margin:'0 auto', display:'flex', flexDirection:'column', gap:8}}>
          ...
          <label>Empresa</label>
          <select name="id_empresa" value={form.id_empresa} onChange={handleChange} required className="input">
          ...
          <input name="nombre_software" value={form.nombre_software} onChange={handleChange} className="input" />
          <label>Usuario</label>
          <select name="id_usuario" value={form.id_usuario} onChange={handleChange} required className="input" disabled={!!editId}>
          ...
          <select name="id_metodologia" value={form.id_metodologia} onChange={handleChange} required className="input" disabled={!!editId}>
          ...
          <button type="submit" disabled={loading} className="btn">{loading ? "Guardando..." : "Guardar cambios"}</button>
          <button type="button" onClick={()=>{setEditId(null);setForm({ id_empresa: "", fecha: "", ciudad: "", nombre_software: "", id_usuario: "", id_metodologia: "" });}} className="btn" style={{background:'#e5e7eb', color:'#222', marginTop:8}}>Cancelar</button>
        </form>
      )}
    </div>
  );
